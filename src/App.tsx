...
export default function App() {
  ...
  const handleShareReport = async () => {
    ...
    try {
      // Ensure hidden report is rendered correctly
      ...
      const file = new File([blob], `Report_Changeshift_${new Date().toLocaleDateString()}.png`, { type: 'image/png' });

      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Changeshift-Tracker Report',
          text: `Laporan Changeshift Monitoring - ${new Date().toLocaleDateString('id-ID')}`,
        });
      } else {
        ...
        a.href = url;
        a.download = `Changeshift_Report_${Date.now()}.png`;
        a.click();
        URL.revokeObjectURL(url);
        alert('Sharing tidak didukung di browser ini. Gambar telah diunduh.');
      }
    } catch (error) {
          ...
          {history.length > 0 && (
            <div className="mt-6 pt-6 border-t border-slate-800 flex flex-col gap-3">
              <button 
                ...
                disabled={isSharing}
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-900/50 text-white py-4 rounded-xl font-bold uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-lg shadow-emerald-900/20"
              >
                {isSharing ? (
              ...
              <p className="text-[9px] text-slate-600 text-center font-mono uppercase tracking-widest">
                Membagikan laporan dalam bentuk gambar (.png)
              </p>
            </div>
          )}
        </section>
